...
def save_output_image(output, output_name, pixel_means):
    ...
    # Save the image
    Image.fromarray(
        (output[0].permute(1, 2, 0).numpy() + pixel_means)[:, :, ::-1]
        .clip(0, 255)
        .astype(np.uint8)
    ).save(output_name)
...
def process_style_transfer(config: StyleTransferConfig):
    ...
    device = torch.device(
        "cuda" if torch.cuda.is_available() and str(config.device).isdigit() else "cpu"
    )
    ...
    for content_path in content_paths:
        if do_interpolation:  # one content image, N style image
            ...
            output_name = os.path.join(
                config.output, f"{content_path.stem}_interpolation{config.save_ext}"
            )
            ...
            for style_path in style_paths:
                ...
                output = output.cpu()
                output_name = os.path.join(
                    config.output, f"{content_path.stem}{config.save_ext}"
                )
                save_output_image(output, output_name, pixel_means)
...
if __name__ == "__main__":
    ...
    # Additional options
    parser.add_argument(
        "--content_size", type=int, default=0, help="Size of content images"
    )
    parser.add_argument(
        "--style_size", type=int, default=0, help="Size of style images"
    )
    parser.add_argument(
        "--crop", action="store_true", help="If specified, crop images to square"
    )
    parser.add_argument(
