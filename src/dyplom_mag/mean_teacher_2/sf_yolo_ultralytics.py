...
class SFDetectionModel(DetectionModel):
    ...
    def __init__(
        self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True
...
class SFYOLOTrainer:
    ...
    def __init__(
        ...
        if encoder_path:
            ...
            self.encoder_path = os.path.join(
                BASE_DIR, "..", "target_augment", "pre_trained", "vgg16_ori.pth"
            )
        if decoder_path:
            self.decoder_path = decoder_path
        else:
            self.decoder_path = os.path.join(
                BASE_DIR, "..", "target_augment", "models", "decoder.pth"
            )
        if fc1_path:
            self.fc1_path = fc1_path
        else:
            self.fc1_path = os.path.join(
                BASE_DIR, "..", "target_augment", "models", "fc1.pth"
            )
        if fc2_path:
            self.fc2_path = fc2_path
        else:
            self.fc2_path = os.path.join(
                BASE_DIR, "..", "target_augment", "models", "fc2.pth"
            )
        ...
        self.data_path = Path(
            data_dir or self.data_dict.get("path") or Path(self.data_yaml).parent
        )
        ...
        for k in ["train", "val", "test", "minival"]:
            if k in self.data_dict and self.data_dict[k]:
                if isinstance(self.data_dict[k], str):
                    ...
                    self.data_dict[k] = [
                        str((self.data_path / x).resolve()) for x in self.data_dict[k]
                    ]
    ...
    def train(self):
        ...
        print(
            f"Teacher model device: {next(self.teacher_model.model.parameters()).device}"
        )
        print(
            f"Student model device: {next(self.student_model.model.parameters()).device}"
        )
        ...
        trainable_params = sum(
            p.numel() for p in self.student_model.model.parameters() if p.requires_grad
        )

        if trainable_params == 0:
            print(
                "No trainable parameters found in student model. Setting detection head to trainable."
            )
            ...
            trainable_params = sum(
                p.numel()
                for p in self.student_model.model.parameters()
                if p.requires_grad
            )
            ...
            if trainable_params == 0:
                print(
                    "Still no trainable parameters. Setting all parameters to trainable."
                )
                ...
                if trainable_params == 0:
                    raise ValueError(
                        "Failed to set any parameters to trainable in student model"
                    )
        ...
        student_params = [
            p for p in self.student_model.model.parameters() if p.requires_grad
        ]
        ...
        if len(teacher_params) != len(student_params):
            print(
                f"Warning: Teacher and student parameter counts don't match: {len(teacher_params)} vs {len(student_params)}"
            )
            ...
            teacher_optimizer = WeightEMA(
                teacher_params, student_params_all, alpha=self.teacher_alpha
            )
        else:
        ...
        scheduler = torch.optim.lr_scheduler.LambdaLR(
            student_optimizer, lr_lambda=cosine_lr_scheduler
        )
        ...
        for epoch in range(self.epochs):
            ...
            if epoch > 0 and self.ssm_alpha > 0:
                ...
                print(
                    f"Updated student model with teacher weights (alpha={self.ssm_alpha})"
                )
            ...
            for i, batch in pbar:
                ...
                if self.style_transfer:
                    imgs_style = (
                        get_style_images(imgs_255, adain=self.style_transfer) / 255.0
                    )
                else:
                ...
                with torch.no_grad():
                    ...
                    if i == 0:
                        ...
                        print(
                            f"Model device: {next(self.teacher_model.model.parameters()).device}"
                        )
                ...
                if not pseudo_labels:
                    ...
                    pseudo_labels = [
                        torch.tensor([[0, 0, 0.5, 0.5, 0.1, 0.1]], device=self.device)
                    ]
            ...
            print(
                f"Epoch {epoch + 1} results - mAP50-95: {current_map:.4f}, mAP50: {metrics.get('metrics/mAP50', 0):.4f}"
            )
            ...
            if current_map > self.best_map:
                ...
                # Store best weights
                self.best_teacher_weights = copy.deepcopy(
                    self.teacher_model.model.state_dict()
                )
                self.best_student_weights = copy.deepcopy(
                    self.student_model.model.state_dict()
                )
        ...
        print(
            f"Best model was at epoch {self.best_epoch + 1} with mAP50-95: {self.best_map:.4f}"
        )
    ...
    def _create_dataloader(self):
        ...
        try:
            ...
            stride = 32
            LOGGER.warning(
                f"WARNING ⚠️ Unable to determine stride, using default value {stride}."
            )
        ...
        loader = build_dataloader(
            dataset=dataset, batch=self.batch_size, workers=8, shuffle=True, rank=-1
        )
    ...
    def _save_results(self):
        ...
        if self.metrics_history:
            ...
            if all("metrics/precision" in m for m in self.metrics_history):
                precision = [
                    m.get("metrics/precision", 0) for m in self.metrics_history
                ]
